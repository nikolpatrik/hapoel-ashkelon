...
export default function Home() {
  return (
    <main dir="rtl" className="min-h-screen bg-[#F5F7FA] text-[#17263d]">
      ...
      <section className="relative -mt-10 px-6"><div className="mx-auto grid max-w-6xl gap-4 md:grid-cols-3"><Stat value="4" label="ענפי ספורט פעילים באתר החדש" /><Stat value="אשקלון" label="ספורטאים שמייצגים את העיר" /><Stat value="מצוינות" label="קהילה • ספורט • הישגים" /></div></section>
      <section id="sports" className="px-6 py-24"><div className="mx-auto max-w-7xl"><div className="text-center"><span className="text-sm font-bold text-[#18b6b4]">OUR SPORTS</span><h2 className="mt-3 text-3xl font-black text-[#102f47] md:text-5xl">ענפי הספורט שלנו</h2><p className="mx-auto mt-5 max-w-2xl leading-8 text-slate-500">האתר המקורי מציג ארבעה ענפים מרכזיים: רוגבי, טניס שולחן, אגרוף וסיוף.</p></div><div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{sports.map((sport) => (<a key={sport.href} href={sport.href} className="group overflow-hidden rounded-3xl bg-white shadow-md transition duration-300 hover:-translate-y-2 hover:shadow-xl"><div className="flex h-52 w-full items-center justify-center bg-[#f5f7fa] p-6"><img src={`${RAW}${sport.image}`} alt={sport.title} className="h-full w-full object-contain transition duration-300 group-hover:scale-105" /></div><div className="p-7"><div className="text-2xl">{sport.icon}</div><h3 className="mt-2 text-2xl font-black text-[#102f47]">{sport.title}</h3><p className="mt-3 leading-7 text-slate-500">{sport.description}</p><div className="mt-6 font-bold text-[#18b6b4]">למידע נוסף ←</div></div></a>))}</div></div></section>
      <section id="about" className="bg-[#102f47] px-6 py-24 text-white"><div className="mx-auto max-w-6xl text-center"><span className="text-sm font-bold text-[#7ee7e5]">מי אנחנו</span><h2 className="mt-3 text-3xl font-black md:text-5xl">גוף ספורט שמייצג את אשקלון</h2><p className="mx-auto mt-7 max-w-3xl text-lg leading-9 text-slate-300">לפי האתר המקורי, העמותה מוכרת על ידי הגופים המוסמכים בישראל ועל ידי עיריית אשקלון ומפעילה מועדוני ספורט בענפים שונים. הספורטאים והקבוצות מייצגים את העיר, והמצליחים שבהם מגיעים לנבחרות ישראל ולתחרויות בינלאומיות.</p><a href="/about" className="mt-8 inline-flex rounded-full border border-white/30 px-8 py-4 font-bold hover:bg-white/10">קראו עוד על העמותה</a></div></section>
      <section className="px-6 py-24"><div className="mx-auto max-w-6xl rounded-[32px] bg-white p-9 text-center shadow-xl md:p-14"><span className="text-sm font-bold text-[#18b6b4]">שותפינו</span><h2 className="mt-3 text-3xl font-black text-[#102f47]">תחנה לרפואת ספורט</h2><p className="mx-auto mt-5 max-w-2xl leading-8 text-slate-500">האתר המקורי מציין את תחנה לרפואת ספורט בין השותפים לעמותה.</p></div></section>
      <section id="contact" className="px-6 pb-24"><div className="mx-auto max-w-4xl rounded-[2rem] bg-white p-10 text-center shadow-xl md:p-16"><h2 className="text-3xl font-black text-[#102f47] md:text-4xl">רוצים להיות חלק?</h2><p className="mx-auto mt-5 max-w-2xl leading-8 text-slate-500">רוצים להצטרף לפעילות, לשיעור ניסיון או לקבל מידע נוסף? השאירו פרטים או פנו אלינו ישירות בוואטסאפ.</p><div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row"><a href="/leave-details" className="inline-flex rounded-full bg-[#18b6b4] px-8 py-4 font-bold text-white transition hover:bg-[#129da0]">השאירו פרטים</a><a href="https://wa.me/972526781740" target="_blank" rel="noopener noreferrer" className="inline-flex rounded-full border-2 border-[#18b6b4] px-8 py-4 font-bold text-[#102f47] transition hover:bg-[#18b6b4]/10" aria-label="פתיחת וואטסאפ עם ניקול">WhatsApp · ניקול</a></div><div className="mt-7 text-sm text-slate-500"><span>ניקול · </span><a href="https://wa.me/972526781740" target="_blank" rel="noopener noreferrer" className="font-bold text-[#18b6b4] hover:underline" dir="ltr">052-6781740</a></div></div></section>
