...
const weapons = [
  {
    ...
    video: "/api/fencing-video?pathname=fencing_2.mp4"
  },
  {
    title: "חרב",
    en: "Sabre",
    ...
    video: "/api/fencing-video?pathname=fencing_3.mp4"
  },
  {
    title: "רומח",
    en: "Foil",
    ...
    video: "/api/fencing-video?pathname=fencing_1.mp4"
  },
];
    ...
    <main dir="rtl" className="page">
      ...
      <section className="intro section">
        ...
        <div className="intro-grid">
          <div><h2>קרב בין שני אנשים.<br /><em>משחק שחמט במהירות גבוהה.</em></h2></div>
          <div className="intro-text"><p>הסייף המודרני מתקיים על מסלול ייעודי, בין שני ספורטאים המצוידים בביגוד מגן ובמערכת אלקטרונית המזהה פגיעות.</p><p>בכל קרב הסייף צריך לקרוא את היריב, לשלוט במרחק, לבחור את הרגע הנכון ולשלב בין התקפה, הגנה ותנועה.</p></div>
        </div>
      </section>
      <section className="weapons section dark-section">
        <div className="section-heading"><div><div className="section-label">02 · שלוש ההתמחויות</div><h2>שלוש דרכים <span>לסייף.</span></h2></div><p>כל כלי מספר סיפור אחר — חוקים, קצב וסגנון משחק שונים.</p></div>
        <div className="weapon-grid">
          {weapons.map((weapon) => (
            <article className="weapon-card" key={weapon.title} tabIndex={0} onMouseEnter={() => playVideo(weapon.title)} onMouseLeave={() => stopVideo(weapon.title)} onFocus={() => playVideo(weapon.title)} onBlur={() => stopVideo(weapon.title)}>
              <img className="weapon-image" src={weapon.image} alt={weapon.title} />
              <video ref={(node) => { videoRefs.current[weapon.title] = node; }} className="weapon-video" src={weapon.video} muted loop playsInline preload="metadata" aria-hidden="true" />
      ...
      <section className="pillars section">
        <div className="section-label">03 · מה הסייף מפתח?</div><h2>הגוף מתאמן.<br /><span>גם המוח.</span></h2>
        <div className="pillar-grid">{pillars.map((item) => <article key={item.title}><div className="pillar-icon">{item.icon}</div><h3>{item.title}</h3><p>{item.text}</p></article>)}</div>
      </section>
      <section className="timeline section dark-section">
        ...
        <div className="timeline-grid">{milestones.map((m) => <article key={m.year}><strong>{m.year}</strong><div className="line" /><h3>{m.title}</h3><p>{m.text}</p></article>)}</div>
      </section>
      <section className="israel section">
        <div className="israel-image" /><div className="israel-content"><div className="section-label">05 · הסייף בישראל</div><h2>מסורת שהפכה<br /><span>להישגים כחול־לבן.</span></h2><p>הסייף בישראל התפתח לאורך עשרות שנים, ממועדונים קטנים ומאמנים שהביאו ידע מאירופה לענף תחרותי שמופיע על הבמות הגדולות בעולם.</p><div className="achievements">{israel.map((item, i) => <div key={i}><b>{String(i + 1).padStart(2, "0")}</b><span>{item}</span></div>)}</div></div>
      </section>
